<template>
    <header
        ...
        <div class="flex items-center gap-2 lg:gap-4">
            <!-- Search -->
            <div
                class="hidden sm:flex items-center bg-background border border-border rounded-lg px-3 py-2 w-40 md:w-56 focus-within:border-primary-400 transition-colors">
            ...
            <div class="relative">
                <button @click="isProfileOpen = !isProfileOpen"
                    ...
                    <div
                        class="w-8 h-8 rounded-full bg-primary-400 flex items-center justify-center text-white text-sm font-semibold shrink-0">
                        ع
                    </div>
                    <span class="hidden md:block text-sm text-text font-medium">{{ authStore.user?.name + ' ' +
                        authStore.user?.last_name }}</span>
                    <svg class="hidden md:block w-4 h-4 text-muted transition-transform duration-200"
                        :class="isProfileOpen ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                ...
                <div v-if="isProfileOpen"
                    ...
                    <NuxtLink class="block px-4 py-2 text-sm text-text hover:bg-background transition-colors" to="/">
                        بازگشت به سایت</NuxtLink>
                    <a href="#"
                        class="block px-4 py-2 text-sm text-text hover:bg-background transition-colors">پروفایل</a>
                    <a href="#"
                        class="block px-4 py-2 text-sm text-text hover:bg-background transition-colors">تنظیمات</a>
                    <hr class="my-1 border-border" />
                    <a href="#"
                        class="block px-4 py-2 text-sm text-danger hover:bg-danger/10 transition-colors">خروج</a>
                </div>
            </div>
        </div>
    </header>
